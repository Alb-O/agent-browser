...
pub fn find_installed_chrome() -> Option<PathBuf> {
	...
	let mut versions: Vec<_> = fs::read_dir(&browsers_dir)
		.ok()?
		.filter_map(|e| e.ok())
		.filter(|e| e.file_name().to_str().is_some_and(|n| n.starts_with("chrome-")))
		.collect();
...
async fn download_bytes(url: &str) -> Result<Vec<u8>, String> {
	...
	loop {
		let chunk = stream.chunk().await.map_err(|e| format!("Download error: {}", e))?;
		match chunk {
			Some(data) => {
...
fn install_linux_deps() {
	...
	let install_cmd = match pkg_mgr {
		"apt-get" => {
			format!("sudo apt-get update && sudo apt-get install -y {}", deps.join(" "))
		}
