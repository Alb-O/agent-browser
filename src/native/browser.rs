use {
	super::cdp::{
		chrome::{ChromeProcess, LaunchOptions, auto_connect_cdp, launch_chrome},
		client::CdpClient,
		discovery::discover_cdp_url,
		lightpanda::{LightpandaLaunchOptions, LightpandaProcess, launch_lightpanda},
		types::*,
	},
...
pub fn validate_launch_options(
	extensions: Option<&[String]>, has_cdp: bool, profile: Option<&str>, storage_state: Option<&str>,
	allow_file_access: bool, executable_path: Option<&str>,
) -> Result<(), String> {
...
impl BrowserManager {
	...
	async fn wait_for_lifecycle(
		&self, wait_until: WaitUntil, session_id: &str, rx: &mut broadcast::Receiver<CdpEvent>,
	) -> Result<(), String> {
	...
	async fn wait_for_network_idle(
		&self, session_id: &str, rx: &mut broadcast::Receiver<CdpEvent>,
	) -> Result<(), String> {
	...
	pub async fn is_connection_alive(&self) -> bool {
		...
		let result =
			tokio::time::timeout(timeout, self.client.send_command_no_params("Browser.getVersion", None)).await;
	...
	pub async fn set_viewport(
		&self, width: i32, height: i32, device_scale_factor: f64, mobile: bool,
	) -> Result<(), String> {
	...
	pub async fn set_emulated_media(
		&self, media: Option<&str>, features: Option<Vec<(String, String)>>,
	) -> Result<(), String> {
		let session_id = self.active_session_id()?;
		let mut params = json!({});
		if let Some(m) = media {
	...
	pub async fn upload_files(&self, selector: &str, files: &[String]) -> Result<(), String> {
		...
		let object_id = result.result.object_id.ok_or("File input element not found")?;
...
async fn connect_cdp_with_retry(
	ws_url: &str, total_timeout: Duration, poll_interval: Duration,
...
async fn run_with_lightpanda_deadline<F, T>(
	deadline: Instant, operation: F, timeout_context: &'static str,
) -> Result<T, String>
where
