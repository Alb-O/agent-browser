use {
	super::discovery::discover_cdp_url,
	std::{
		...
		if let Some(ref dir) = self.temp_user_data_dir {
			for attempt in 0..3 {
				match std::fs::remove_dir_all(dir) {
					...
					Err(e) => {
						eprintln!("Warning: failed to clean up temp profile {}: {}", dir.display(), e);
					}
				}
			}
		}
...
pub fn launch_chrome(options: &LaunchOptions) -> Result<ChromeProcess, String> {
	let chrome_path = match &options.executable_path {
		...
		None => find_chrome()
			.ok_or("Chrome not found. Run `agent-browser install` to download Chrome, or use --executable-path.")?,
	};
