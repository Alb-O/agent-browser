use {super::types::BrowserVersionInfo, std::time::Duration};
...
pub async fn discover_cdp_url_with_request_timeout(port: u16, request_timeout: Duration) -> Result<String, String> {
...
mod tests {
	...
	async fn discovers_ws_url_from_json_version() {
		let (port, server) = spawn_json_server(r#"{"webSocketDebuggerUrl":"ws://127.0.0.1:1234/"}"#).await;
