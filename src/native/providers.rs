...
pub async fn connect_provider(provider_name: &str) -> Result<(String, Option<ProviderSession>), String> {
...
async fn connect_browserless() -> Result<(String, Option<ProviderSession>), String> {
	...
	let api_url =
		env::var("BROWSERLESS_API_URL").unwrap_or_else(|_| "https://production-sfo.browserless.io".to_string());
...
async fn connect_browser_use() -> Result<(String, Option<ProviderSession>), String> {
	...
	if !status.is_success() {
		return Err(format!("Browser Use API error ({}): {}", status.as_u16(), body));
	}

	let json: Value = serde_json::from_str(&body).map_err(|e| format!("Invalid Browser Use response: {}", e))?;
