...
pub fn recording_add_frame(state: &mut RecordingState, frame_data: &[u8]) {
	...
	let frame_path = state.temp_dir.join(format!("frame_{:06}.jpg", state.frame_count));
...
pub fn recording_stop(state: &mut RecordingState) -> Result<Value, String> {
	...
	let frame_pattern = state.temp_dir.join("frame_%06d.jpg").to_string_lossy().to_string();
