...
fn encrypt_profile(profile: &AuthProfile) -> Result<String, String> {
	...
	let mut iv = [0u8; 12];
	getrandom::fill(&mut iv).map_err(|e| format!("Failed to generate IV: {}", e))?;
...
pub fn auth_save(
	name: &str, url: &str, username: &str, password: &str, username_selector: Option<&str>,
	password_selector: Option<&str>, submit_selector: Option<&str>,
) -> Result<Value, String> {
	validate_profile_name(name)?;
	let profile = AuthProfile {
		name: name.to_string(),
