use {
	super::cdp::{client::CdpClient, types::*},
	serde_json::Value,
...
async fn resolve_by_role_name(
	client: &CdpClient, session_id: &str, role: &str, name: &str, nth: Option<usize>,
) -> Result<(f64, f64), String> {
	...
	match (x, y) {
		...
		_ => Err(format!("Could not locate element with role={} name={}", role, name)),
	}
}
...
pub async fn get_element_attribute(
	client: &CdpClient, session_id: &str, ref_map: &RefMap, selector_or_ref: &str, attribute: &str,
) -> Result<Value, String> {
	let object_id = resolve_element_object_id(client, session_id, ref_map, selector_or_ref).await?;
...
pub async fn get_element_input_value(
	...
	let result: EvaluateResult = client
		.send_command_typed(
			"Runtime.callFunctionOn",
			&CallFunctionOnParams {
				function_declaration: "function() { return typeof this.value === 'string' ? this.value : ''; }"
					.to_string(),
				object_id: Some(object_id),
				arguments: None,
				return_by_value: Some(true),
...
pub async fn get_element_styles(
	client: &CdpClient, session_id: &str, ref_map: &RefMap, selector_or_ref: &str, properties: Option<Vec<String>>,
) -> Result<Value, String> {
	let object_id = resolve_element_object_id(client, session_id, ref_map, selector_or_ref).await?;
