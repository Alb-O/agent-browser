use {
	super::{
		...
		cdp::client::CdpClient,
		state,
		stream::StreamServer,
	},
	serde_json::Value,
	std::{env, fs, path::PathBuf, process, sync::Arc, time::Duration},
	tokio::{
...
async fn run_socket_server(
	socket_path: &PathBuf, _session: &str, stream_client: Option<Arc<RwLock<Option<Arc<CdpClient>>>>>,
	...
	let state: std::sync::Arc<tokio::sync::Mutex<DaemonState>> = std::sync::Arc::new(tokio::sync::Mutex::new(
		DaemonState::new_with_stream_client(stream_client),
	));
...
#[cfg(windows)]
async fn run_socket_server(
	socket_path: &PathBuf, session: &str, stream_client: Option<Arc<RwLock<Option<Arc<CdpClient>>>>>,
	idle_timeout_ms: Option<u64>,
) -> Result<(), String> {
...
async fn handle_connection<S>(
	stream: S, state: std::sync::Arc<tokio::sync::Mutex<DaemonState>>, idle_reset_tx: Option<Arc<mpsc::Sender<()>>>,
) where
