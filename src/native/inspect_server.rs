use {
	super::cdp::client::InspectProxyHandle,
...
impl InspectServer {
	...
	pub async fn start(
		proxy_handle: InspectProxyHandle, target_id: String, chrome_host_port: String,
	) -> Result<Self, String> {
...
async fn accept_loop(
	listener: TcpListener, proxy: Arc<InspectProxyHandle>, target_id: String, chrome_host_port: String, proxy_port: u16,
) {
	loop {
...
async fn handle_connection(
	stream: tokio::net::TcpStream, proxy: Arc<InspectProxyHandle>, target_id: String, chrome_host_port: String,
	proxy_port: u16,
) -> Result<(), String> {
...
async fn handle_http_redirect(
	buf_reader: BufReader<tokio::net::TcpStream>, chrome_host_port: String, proxy_port: u16,
) -> Result<(), String> {
...
async fn handle_ws_proxy(
	stream: tokio::net::TcpStream, proxy: Arc<InspectProxyHandle>, target_id: String,
) -> Result<(), String> {
		...
		loop {
			let raw_msg = match raw_rx.recv().await {
				...
				Err(tokio::sync::broadcast::error::RecvError::Lagged(n)) => {
					eprintln!("[inspect] warning: dropped {} CDP messages (channel lag)", n);
					continue;
				}
				Err(_) => break,
			};
			...
			if tx.send(Message::Text(stripped.into())).await.is_err() {
				break;
			}
		}
	});
