use {
	super::{
		cdp::{
			client::CdpClient,
			types::{
				AXNode, AXProperty, AXValue, CallFunctionOnParams, EvaluateParams, EvaluateResult, GetFullAXTreeResult,
			},
		},
		element::RefMap,
	},
...
pub async fn take_snapshot(
	client: &CdpClient, session_id: &str, options: &SnapshotOptions, ref_map: &mut RefMap,
	...
	let ax_tree: GetFullAXTreeResult = client
		.send_command_typed("Accessibility.getFullAXTree", &serde_json::json!({}), Some(session_id))
		.await?;
...
async fn find_cursor_interactive_elements(
	client: &CdpClient, session_id: &str, ref_map: &mut RefMap,
) -> Result<String, String> {
	let js = r#"
(function() {
    const elements = [];
...
"#;
	...
	let result_array = props_result.get("result").and_then(|v| v.as_array()).unwrap_or(&empty);
	...
	let get_text_js = r#"function(){ return (this.innerText || this.textContent || '').trim().slice(0, 100) }"#;
