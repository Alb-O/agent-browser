use {
	super::{
		auth,
		browser::{BrowserManager, WaitUntil},
		cdp::{
			chrome::LaunchOptions,
			client::CdpClient,
			types::{
				AttachToTargetParams, AttachToTargetResult, CdpEvent, ConsoleApiCalledEvent, CreateTargetResult,
				ExceptionThrownEvent, TargetCreatedEvent, TargetDestroyedEvent,
			},
		},
		cookies, diff,
		element::RefMap,
		inspect_server::InspectServer,
		interaction,
		network::{self, DomainFilter, EventTracker},
		policy::{ActionPolicy, ConfirmActions, PolicyResult},
		providers,
		recording::{self, RecordingState},
		screenshot::{self, ScreenshotOptions},
		snapshot::{self, SnapshotOptions},
		state, storage, stream,
		tracing::{self as native_tracing, TracingState},
		webdriver::{
			appium::AppiumManager,
			backend::{BrowserBackend, WEBDRIVER_UNSUPPORTED_ACTIONS, WebDriverBackend},
			ios, safari,
		},
	},
...
impl DaemonState {
	...
	pub fn new_with_stream_client(stream_client: Option<Arc<RwLock<Option<Arc<CdpClient>>>>>) -> Self {
		let mut s = Self::new();
	...
	fn drain_cdp_events(&mut self) -> (Vec<i64>, Vec<TargetCreatedEvent>, Vec<String>, Vec<FetchPausedRequest>) {
				...
				Ok(event) => {
					...
					match event.method.as_str() {
						"Runtime.consoleAPICalled" => {
							if let Ok(console_event) =
								serde_json::from_value::<ConsoleApiCalledEvent>(event.params.clone())
							{
								let text: String = console_event
									...
									.collect::<Vec<_>>()
									.join(" ");
								self.event_tracker.add_console(&console_event.call_type, &text);
							}
						}
						...
						"Network.responseReceived" if self.har_recording => {
							if let Some(response) = event.params.get("response") {
								...
								let mime_type = response.get("mimeType").and_then(|v| v.as_str()).map(String::from);
								if let Some(entry) =
									self.har_entries.iter_mut().rev().find(|e| e.request_id == request_id)
								{
									entry.status = status;
...
pub async fn execute_command(cmd: &Value, state: &mut DaemonState) -> Value {
		...
		if let Some(ref mut mgr) = state.browser {
			...
			if let Ok(attach) = attach_result {
				...
				if let Some(ref filter) = state.domain_filter {
					let _ =
						network::install_domain_filter(&mgr.client, &attach.session_id, &filter.allowed_domains).await;
				}
	...
	for paused in &fetch_paused {
		if let Some(ref browser) = state.browser {
			resolve_fetch_paused(browser, state.domain_filter.as_ref(), &state.routes, paused).await;
		}
	}
...
async fn handle_launch(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let allow_file_access = cmd.get("allowFileAccess").and_then(|v| v.as_bool()).unwrap_or(false);
...
async fn handle_snapshot(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let options = SnapshotOptions {
		...
		depth: cmd.get("depth").and_then(|v| v.as_u64()).map(|d| d as usize),
		cursor: cmd.get("cursor").and_then(|v| v.as_bool()).unwrap_or(false),
	};

	state.ref_map.clear();
	let tree = snapshot::take_snapshot(&mgr.client, &session_id, &options, &mut state.ref_map).await?;
...
async fn handle_screenshot(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let result = screenshot::take_screenshot(&mgr.client, &session_id, &state.ref_map, &options).await?;
...
async fn handle_click(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	if new_tab {
		...
		let object_id = resolve_element_object_id(&mgr.client, &session_id, &state.ref_map, selector).await?;
	...
	interaction::click(&mgr.client, &session_id, &state.ref_map, selector, button, click_count).await?;
...
async fn handle_type(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	interaction::type_text(&mgr.client, &session_id, &state.ref_map, selector, text, clear, delay).await?;
	Ok(json!({ "typed": text }))
}
...
async fn handle_gettext(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let text = super::element::get_element_text(&mgr.client, &session_id, &state.ref_map, selector).await?;
	let url = mgr.get_url().await.unwrap_or_default();
...
async fn handle_getattribute(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let value =
		super::element::get_element_attribute(&mgr.client, &session_id, &state.ref_map, selector, attribute).await?;
	let url = mgr.get_url().await.unwrap_or_default();
...
async fn handle_isvisible(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let visible = super::element::is_element_visible(&mgr.client, &session_id, &state.ref_map, selector).await?;
	let url = mgr.get_url().await.unwrap_or_default();
...
async fn handle_isenabled(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let enabled = super::element::is_element_enabled(&mgr.client, &session_id, &state.ref_map, selector).await?;
	let url = mgr.get_url().await.unwrap_or_default();
...
async fn handle_ischecked(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let checked = super::element::is_element_checked(&mgr.client, &session_id, &state.ref_map, selector).await?;
	let url = mgr.get_url().await.unwrap_or_default();
...
async fn wait_for_selector(
	client: &super::cdp::client::CdpClient, session_id: &str, selector: &str, state: &str, timeout_ms: u64,
) -> Result<(), String> {
...
async fn wait_for_url(
	client: &super::cdp::client::CdpClient, session_id: &str, pattern: &str, timeout_ms: u64,
) -> Result<(), String> {
	let check_fn = format!(
...
async fn wait_for_text(
	client: &super::cdp::client::CdpClient, session_id: &str, text: &str, timeout_ms: u64,
) -> Result<(), String> {
	let check_fn = format!(
...
async fn wait_for_function(
	client: &super::cdp::client::CdpClient, session_id: &str, fn_str: &str, timeout_ms: u64,
) -> Result<(), String> {
...
async fn poll_until_true(
	client: &super::cdp::client::CdpClient, session_id: &str, expression: &str, timeout_ms: u64,
) -> Result<(), String> {
...
async fn handle_diff_snapshot(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let current = snapshot::take_snapshot(&mgr.client, &session_id, &options, &mut state.ref_map).await?;
...
async fn handle_diff_url(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let snap1 = snapshot::take_snapshot(&mgr.client, &session_id, &options, &mut state.ref_map).await?;

	// Navigate to URL2 and snapshot
	mgr.navigate(url2, wait_until).await?;
	state.ref_map.clear();
	let snap2 = snapshot::take_snapshot(&mgr.client, &session_id, &options, &mut state.ref_map).await?;
...
async fn handle_boundingbox(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let bbox = super::element::get_element_bounding_box(&mgr.client, &session_id, &state.ref_map, selector).await?;
	Ok(bbox)
}
...
async fn handle_innertext(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let text = super::element::get_element_inner_text(&mgr.client, &session_id, &state.ref_map, selector).await?;
	Ok(json!({ "text": text }))
}
...
async fn handle_innerhtml(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let html = super::element::get_element_inner_html(&mgr.client, &session_id, &state.ref_map, selector).await?;
	Ok(json!({ "html": html }))
}
...
async fn handle_inputvalue(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let value = super::element::get_element_input_value(&mgr.client, &session_id, &state.ref_map, selector).await?;
	Ok(json!({ "value": value }))
}
...
async fn handle_setvalue(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	super::element::set_element_value(&mgr.client, &session_id, &state.ref_map, selector, value).await?;
...
async fn handle_styles(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let styles =
		super::element::get_element_styles(&mgr.client, &session_id, &state.ref_map, selector, properties).await?;
...
async fn handle_device(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
	...
	let (width, height, scale, mobile, ua) = match name.to_lowercase().as_str() {
		...
		_ => {
			return Err(format!(
				"Unknown device: {}. Supported: iPhone 12, iPhone 14, iPhone 15, iPad, iPad Pro, Pixel 5, Pixel 7, Galaxy S21",
				name
			));
		}
	};
...
async fn execute_subaction(cmd: &Value, state: &mut DaemonState, selector: &str) -> Result<Value, String> {
	...
	match subaction {
		"click" => {
			interaction::click(&mgr.client, &session_id, &state.ref_map, selector, "left", 1).await?;
			Ok(json!({ "clicked": selector }))
		}
		"fill" => {
			let value = cmd
...
async fn resolve_semantic_locator(
	client: &super::cdp::client::CdpClient, session_id: &str, strategy: &str, value: &str, exact: bool,
	...
	if result.result.value.as_ref().map(|v| v.is_null()).unwrap_or(true) {
...
async fn handle_semantic_locator(
	cmd: &Value, state: &mut DaemonState, strategy: &str, param_name: &str,
) -> Result<Value, String> {
	let mgr = state.browser.as_ref().ok_or("Browser not launched")?;
	let session_id = mgr.active_session_id()?.to_string();
	let value = cmd
...
async fn handle_drag(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let (sx, sy) = super::element::resolve_element_center(&mgr.client, &session_id, &state.ref_map, source).await?;
	let (tx, ty) = super::element::resolve_element_center(&mgr.client, &session_id, &state.ref_map, target).await?;
...
async fn handle_diff_screenshot(cmd: &Value, state: &DaemonState) -> Result<Value, String> {
	...
	if let (Some(out_path), Some(diff_data)) = (output_path, &result.diff_image) {
...
async fn resolve_fetch_paused(
	browser: &BrowserManager, domain_filter: Option<&DomainFilter>, routes: &[RouteEntry], paused: &FetchPausedRequest,
) {
...
async fn handle_route(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	mgr.client
		.send_command("Fetch.enable", Some(json!({ "patterns": patterns })), Some(&session_id))
		.await?;
...
async fn handle_requests(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	let requests: Vec<&TrackedRequest> = if let Some(f) = filter {
		state.tracked_requests.iter().filter(|r| r.url.contains(f)).collect()
	} else {
...
async fn handle_auth_login(cmd: &Value, state: &mut DaemonState) -> Result<Value, String> {
	...
	interaction::fill(&mgr.client, &session_id, &state.ref_map, &user_sel, &username).await?;

	// Find and fill password
	let pass_sel = password_sel.unwrap_or_else(|| "input[type=password]".to_string());
	interaction::fill(&mgr.client, &session_id, &state.ref_map, &pass_sel, &password).await?;
	...
	interaction::click(&mgr.client, &session_id, &state.ref_map, &sub_sel, "left", 1).await?;
...
#[cfg(test)]
mod tests {
	use {super::*, crate::test_utils::EnvGuard};
