...
use {
	super::actions::{DaemonState, execute_command},
...
async fn e2e_tabs() {
	...
	// Switch to first tab
	let resp = execute_command(&json!({ "id": "6", "action": "tab_switch", "index": 0 }), &mut state).await;
	assert_success(&resp);
	...
	// Close second tab
	let resp = execute_command(&json!({ "id": "8", "action": "tab_close", "index": 1 }), &mut state).await;
	assert_success(&resp);
