...
pub async fn take_screenshot(
	client: &CdpClient, session_id: &str, ref_map: &RefMap, options: &ScreenshotOptions,
...
async fn collect_annotations(
	client: &CdpClient, session_id: &str, ref_map: &RefMap,
) -> Result<Vec<RawAnnotation>, String> {
	let mut annotations = Vec::new();

	for (ref_id, entry) in ref_map.entries_sorted() {
		let object_id = match super::element::resolve_element_object_id(client, session_id, ref_map, &ref_id).await {
			Ok(id) => id,
			Err(_) => continue,
		};
...
async fn get_rect_for_selector(
	client: &CdpClient, session_id: &str, ref_map: &RefMap, selector: &str,
) -> Result<Option<Rect>, String> {
	let object_id = super::element::resolve_element_object_id(client, session_id, ref_map, selector).await?;
...
async fn inject_annotation_overlay(
	client: &CdpClient, session_id: &str, annotations: &[RawAnnotation],
) -> Result<(), String> {
...
fn project_annotations(
	annotations: &[RawAnnotation], target_rect: Option<&Rect>, scroll: Option<(f64, f64)>,
...
fn save_screenshot(
	base64_data: &str, explicit_path: Option<&str>, ext: &str, output_dir: Option<&str>,
) -> Result<String, String> {
