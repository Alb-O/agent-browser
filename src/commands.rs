use {
	crate::{
		color,
		flags::Flags,
		validation::{is_valid_session_name, session_name_error},
	},
	...
	match *locator {
		"role" | "text" | "label" | "placeholder" | "alt" | "title" | "testid" | "first" | "last" => {
			...
			match *locator {
				...
				"first" => {
					let mut cmd =
						json!({ "id": id, "action": "nth", "selector": value, "index": 0, "subaction": subaction });
					if let Some(v) = fill_value {
						cmd["value"] = json!(v);
					}
					Ok(cmd)
				...
				"last" => {
					let mut cmd =
						json!({ "id": id, "action": "nth", "selector": value, "index": -1, "subaction": subaction });
					if let Some(v) = fill_value {
						cmd["value"] = json!(v);
					}
					Ok(cmd)
